...
lapply(start_idx:nrow(grid),\(sim_idx) {
  ...
  save_file_lt <- paste0("examples/data/out_simset1_",sim_idx,"_lt.rds")
  ...
  out_actual <- list(theta_mcmc=matrix(c(rep(Phi,actual_k),rep(0,actual_k)),nrow = 2,ncol=actual_k,byrow=TRUE),
                     lambda_mcmc=Lambda)
  ...
  unique_combinations_with_self(types) %>%
    ...
    group_modify(~{
      ...
      tibble(mu=mu,mu_actual,lb=lb,ub=ub,hs=hs,diff=diff,phi=Phi,
...
df_diff %>%
  ...
  # stat_halfeye() +
  # facet_wrap(~hs) +
  ggplot(aes(x=phi,ydist=mean_diff,color=trial_k,fill=trial_k)) +
  stat_halfeye() +
  theme_classic() +
  theme(text=element_text(size=28)) +
  labs(y="MAD between observed and fitted",x="Phi",title = "Simulations with actual k = 3")

df_diff %>%
  mutate(abs_diff=abs(diff)) %>%
  mutate(trial_k=factor(trial_k)) %>%
  mutate(phi_fac= cut(phi,
                         breaks = quantile(phi, probs = seq(0, 1, by = 1/3), na.rm = TRUE),
                         include.lowest = TRUE,
                         labels = c("Low", "Medium", "High"))) %>%
  # mutate(mad=median(abs(diff))) %>%
  group_by(hs,actual_k,trial_k,phi_fac) %>%
  # summarise(mean_diff=mean(mad))
  summarise(mean_diff=rvar_median(abs_diff)) %>%
  # print(n=nrow(.))
  ggplot(aes(xdist=mean_diff,y=trial_k)) +
  stat_halfeye() +
  facet_grid(phi_fac~hs) +
  theme_classic()
