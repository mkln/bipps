...
Bipps::Bipps(
  ...
  if(verbose & debug){
    ...
    Rcpp::Rcout << "Bipps::Bipps initializing took "
...
void Bipps::init_matern(int num_threads, int matern_twonu_in=1, bool use_ps=true){
...
void Bipps::init_meshdata(const arma::mat& theta_in){
  if(verbose & debug){
...
bool Bipps::refresh_cache(BippsDataLMC& data){
...
void Bipps::update_block_covpars(int u, BippsDataLMC& data){
...
void Bipps::update_block_wlogdens(int u, BippsDataLMC& data){
...
bool Bipps::get_loglik_comps_w(BippsDataLMC& data){
...
void Bipps::tausq_update(double new_tausq){
  tausq_inv = 1.0/new_tausq;
}

void Bipps::beta_update(const arma::vec& new_beta){ 
  Bcoeff = new_beta;
}

void Bipps::accept_make_change(){
  std::swap(param_data, alter_data);
}

// --- 
void Bipps::init_for_mcmc(){
  if(verbose & debug){
...
Bipps::Bipps(
  ...
  if(verbose & debug){
    Rcpp::Rcout << "Bipps::Bipps (prior sampling) initialization.\n";
  }
